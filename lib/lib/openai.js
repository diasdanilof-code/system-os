 ...
 * @param {string|object} opts.input - user input (string or object to JSON.stringify)
 * @param {object} opts.schema - JSON schema for structured output
 * @param {string} [opts.schemaName="copilot_response"]
 * @param {string} [opts.model=DEFAULT_MODEL]
 * @param {number} [opts.timeoutMs=DEFAULT_TIMEOUT_MS]
 * @param {string} [opts.reasoning="none"] - reasoning effort (none|low|medium|high|xhigh)
...
export async function callOpenAI({
  ...
  try {
    ...
    // Extract the structured output from the Responses API shape.
    ...
    // Fallback: output_text field some SDKs surface
    if (data.output_text) {
