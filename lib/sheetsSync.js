...
export async function pullAll({ timeoutMs = 20000 } = {}) {
  ...
  try {
    ...
    // DAY_COERCE_PATCH_V1 + BOOL_COERCE_PATCH_V1 — normalize types from Sheets
    const toBool = (v) => {
    ...
    const BOOL_FIELDS = ["closed","morningLogged","afternoonClosed","nightLogged","training","sauna","hbot","redLight","alcohol","fiber","brazilNuts"];
    const NUM_FIELDS = ["weight","sleepH","sleepQ","energy","focus","mood","stress","hunger","cravings","finalEnergy","recovery","diet"];
    if (json.data && Array.isArray(json.data.entries)) {
      json.data.entries = json.data.entries.map((e) => {
        if (!e) return e;
        const dayNum = typeof e.day === "number" ? e.day : Number(e.day);
        const patched = {
          ...e,
        ...
        for (const k of BOOL_FIELDS) {
          if (k in patched) patched[k] = toBool(patched[k]);
        }
        for (const k of NUM_FIELDS) {
          if (k in patched) patched[k] = toNumOrNull(patched[k]);
        }
        return patched;
      });
    }
